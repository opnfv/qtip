...
DATADIR = os.path.join(os.path.dirname(__file__), os.pardir, os.pardir, 'data')


def get_test_plan(name):
    return os.path.join(DATADIR, 'test_plan', name)


def get_output(name):
    return os.path.join(DATADIR, 'output', name)


class TestClass:
    @pytest.mark.parametrize("test_input, expected", [
      (get_test_plan("bm_with_proxy.yaml"), ["dhrystone",
                                         {},
                                         [],
                                         {'http_proxy': 'http://10.20.0.1:8118',
                                         'https_proxy': 'http://10.20.0.1:8118',
                                         'no_proxy': 'localhost,127.0.0.1,10.20.*,192.168.*'}]),
      (get_test_plan("bm_without_proxy.yaml"), ["dhrystone",
                                                   {},
                                                   [],
                                                   {}]),
