...
class TestClass:
    ...
    @pytest.mark.parametrize("test_input, expected", [
        ("tests/data/test_plan/bm_with_proxy.yaml", ["dhrystone",
                                                {},
                                                [],
                                                {'http_proxy': 'http://10.20.0.1:8118',
                                                 'https_proxy': 'http://10.20.0.1:8118',
                                                 'no_proxy': 'localhost,127.0.0.1,10.20.*,192.168.*'}]),
        ("tests/data/test_plan/bm_without_proxy.yaml", ["dhrystone",
                                                   {},
                                                   [],
                                                   {}]),
        ("tests/data/test_plan/vm.yaml", ["iperf",
    ...
    def test_parse_vm_error(self):
        ...
        with pytest.raises(KeyError) as excinfo:
            test_class.parse("tests/data/test_plan/vm_error.yaml")
    ...
    def test_update_ansible(self):
        ...
        mock_ips = mock.Mock(return_value=["10.20.0.28", "10.20.0.29"])
        test_class.fetch_compute_ips = mock_ips
        test_class.parse("tests/data/test_plan/bm_without_proxy.yaml")
        test_class.update_ansible()
        result = filecmp.cmp('tests/data/output/hosts', 'config/hosts')
        assert result
    ...
    def test_ping(self, capfd):
        ...
        test_class.fetch_compute_ips = mock_ips
        test_class.parse("tests/data/test_plan/bm_ping.yaml")
