...
def save_as_baseline(qpi_spec, metrics):
    display.vv("run without baseline, it will not get score")
    display.vv("sort {} metrics".format(qpi_spec['name']))
...
def calc_qpi(qpi_spec, metrics, qpi_baseline):
    ...
    for s in qpi_spec['sections']:
        s_baseline = query(qpi_baseline['sections']).where(
            lambda section: section['name'] == s['name']).to_list()[0]
...
def calc_section(section_spec, metrics, section_baseline):
    ...
    for m in section_spec['metrics']:
        m_baseline = query(section_baseline['metrics']).where(
            lambda metric: metric['name'] == m['name']).to_list()[0]
...
def calc_metric(metric_spec, metrics, metric_basline):
    ...
    for w in metric_spec['workloads']:
        w_baseline = query(metric_basline['workloads']).where(
            lambda workload: workload['name'] == w['name']).to_list()[0]
