...
for x in range (1,11):
	dpi_result_pps=float(os.popen("cat $HOME/qtip_result/dpi_dump.txt | grep 'nDPI throughput:' | awk 'NR=='"+str(x)+" | awk '{print $3}'").read().lstrip())
        dpi_result_bps=float(os.popen("cat $HOME/qtip_result/dpi_dump.txt | grep 'nDPI throughput:' | awk 'NR=='"+str(x)+" | awk '{print $7}'").read().rstrip())
...
os.popen("cat $HOME/qtip_result/dpi_dump.txt > $HOME/qtip_result/"+host+"-"+log_time_stamp+".log")
