...
def setup_test_env(test_case, installer_type):
    try:
    ...
    if doc['Context']['Virtual_Machines']:
        virtual_env = Virtual(doc, installer_type)
        return virtual_env
...
def prepare_ansible_env(benchmark_test_case, installer_type):
    env = setup_test_env(benchmark_test_case, installer_type)
    env.setup()
...
def prepare_and_run_benchmark(installer_type, pwd, benchmark_test_case):
    ...
    benchmark, benchmark_details, proxy_info, env_setup = prepare_ansible_env(benchmark_test_case, installer_type)
