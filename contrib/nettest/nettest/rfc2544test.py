...
spirent:
    ...
    def create_stack(self):
        ...
        if self.affinity:
            self.affinity_sg = self.conn.compute.create_server_group(name=self.AFFINITY_SG_NAME, policies=["affinity"])
        else:
            self.affinity_sg = self.conn.compute.create_server_group(name=self.AFFINITY_SG_NAME,
                                                                     policies=["anti-affinity"])
        self._deploy_test_network()

        user_data = self.STCV_USER_DATA + self.ntp_server_ip

        stcv = self._depoly_stcv(name=self.WEST_STCV_NAME, image_id=image.id, flavor_id=flavor.id,
                                 scheduler_hints=self.affinity_sg, user_data=user_data)
        self.west_stcv = stcv['stcv']
        self.west_stcv_ip = stcv['floating_ip']

        stcv = self._depoly_stcv(name=self.EAST_STCV_NAME, image_id=image.id, flavor_id=flavor.id,
                                 scheduler_hints=self.affinity_sg, user_data=user_data)
    ...
    def delete_stack(self):
        ...
        self.conn.compute.delete_server_group(server_group=self.affinity_sg, ignore_missing=True)
...
class StcRfc2544Test:
    ...
    def do_test(self):
        ...
        self.stc.perform("AttachPorts",
                         params={"portList": self.stc.get("system1.project", "children-port"), "autoConnect": "TRUE"})
    ...
    def format_result(self, metric, original_result_dict):
        ...
        if metric == 'throughput':
            ...
            outputs = original_result_dict["Output"].replace('} {', ',').replace("{", "").replace("}", "").split(",")
