...
class ConsoleReporter(BaseActor):
    ...
    def load_result(self, metric):
        with open(path.join(self.result_path, 'result.json')) as sample:
            result = json.load(sample)
        return result

    def render(self, metric):
        template = self._env.get_template('{}.j2'.format(metric))
        var_dict = self.load_result(metric)
