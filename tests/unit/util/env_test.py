...
def test_check_file_exist(tmpdir):
    exist_file = tmpdir.mkdir('qtip').join('hello.txt')
    exist_file.write("hello")
    non_exist_file = tmpdir.strpath + '/tmp.txt'
    assert env.check_file_exist() is False
    assert env.check_file_exist(str(exist_file))
    assert env.check_file_exist(non_exist_file) is False
    assert env.check_file_exist(str(exist_file), non_exist_file) is False
...
def test_ssh_test(mock_sshclient, stderrinfo, expected):
