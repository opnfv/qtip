...
Since, QTIP is already register on PyPI, now we just need to upload the distributions built in the last step. However,
it is to be noted that only maintainers are permitted to upload the package versions.
