...
from qtip.base.constant import FileName


def fetch_compute_ips_via_installer():
    clean_file(FileName.IPS_FILE)
    ...
    cmd = "bash %s/fetch_compute_ips.sh -i %s -a %s" % \
        (FileName.SCRIPT_DIR, installer_type, installer_ip)
    os.system(cmd)
    if path.isfile(FileName.IPS_FILE):
        return True
    else:
        return False
...
def parse_ips():
    ip_list = []
    with open(FileName.IPS_FILE, "r") as outfile:
        data = outfile.read()
        if data:
...
def ssh_test(ip):
    ...
    ssh_cmd = '%s/qtip_creds.sh %s' % (FileName.SCRIPT_DIR, ip)
    os.system(ssh_cmd)

    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(ip, key_filename='{0}/QtipKey'.format(FileName.CONFIG_DIR))
...
def generate_host_file():
    ip_list = parse_ips()
    with open(FileName.HOST_FILE, 'w') as host_file:
...
def generate_keypair():
    ...
    cmd = "ssh-keygen -t rsa -N "" -f {0} -q".format(FileName.PRIVATE_KEY)
    os.system(cmd)

    if path.isfile(FileName.PRIVATE_KEY) and path.isfile(FileName.PUBLIC_KEY):
        return True
    else:
        return False
...
def clean_keypair():
    flag = True
    flag &= clean_file(FileName.PRIVATE_KEY)
    flag &= clean_file(FileName.PUBLIC_KEY)
    return flag
